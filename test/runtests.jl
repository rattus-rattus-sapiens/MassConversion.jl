...
@testset "MassConversion.jl" begin
    ...
    @testset "Data Structure Tests" begin
        ...
        @test size(model.mean_continuum)         == (5, 1000)
        @test size(model.mean_discrete)          == (5, 1000)
        @test size(model.mean_total)             == (5, 1000)
        @test size(model.var_total)              == (5, 1000)
